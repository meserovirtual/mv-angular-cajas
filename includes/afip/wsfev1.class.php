...
class WSFEV1
{
    const CUIT = 20280799786;                        # CUIT del emisor de las facturas. Solo numeros sin comillas.
    ...
    */
    ...
    private $path = 'c:/xampp/htdocs/afip/'; //caso windows (no importa que las barras esten como en linux)
    ...
    */
    private function _checkErrors($results, $method)
    {
        ...
        $XXX = $method . 'Result';
        if (isset($results->$XXX->Errors) && $results->$XXX->Errors->Err->Code != 0) {
        ...
        if ($method == 'FECAESolicitar') {
            if (isset($results->$XXX->FeDetResp->FECAEDetResponse->Observaciones) && $results->$XXX->FeDetResp->FECAEDetResponse->Observaciones->Obs->Code) {
        ...
        $this->Code = (isset($results->$XXX->Errors))?$results->$XXX->Errors->Err->Code: '';
        $this->Msg = (isset($results->$XXX->Errors))?$results->$XXX->Errors->Err->Msg: '';
        //fin asigna error a variable

        $ret = false;
        if(isset($results->$XXX->Errors)){
            $ret = $results->$XXX->Errors->Err->Code != 0 ? true : false;
        }
        return $ret;
    }
